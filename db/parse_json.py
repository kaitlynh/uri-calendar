...
Key design decisions:
  ...
  - Events are deduplicated on (title, date, time).  When a duplicate is
    found, the existing row is updated only if the incoming data is
    *better*: higher-priority source, more recent scrape, or fewer nulls.
    ...
    for event in events:
        try:
            ...
            cur.execute(
                """
                INSERT INTO events (
                    source_id, source_url, event_title,
                ...
                WHERE
                    ...
                    -- (b) More recent extraction
                    OR EXCLUDED.extracted_at > events.extracted_at
